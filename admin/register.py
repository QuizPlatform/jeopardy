...
for ind in responses.index:
    ...
    db.collection('registered').document(teamCode).set(data, merge=True)
    db.collection('scorecard').document(teamCode).set({'1': 0}, merge=True)
    codes[str(ind)] = teamCode
db.collection('teamCode').document('codes').set(codes, merge=True)
